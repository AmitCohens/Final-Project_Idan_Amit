...
class IMDB:
    ...
    def get_full_description(self,url):
        ...
        try:
            response = self.session.get(url)
            html_content = response.html.xpath("//script[@type='application/json']")[0].text
            result = ''.join(html_content.splitlines())
            result = f"""{result}"""
            result = json.loads(result)
            result = result['props']['pageProps']['contentData']['entityMetadata']['plot']['plotText']['plainText']
            return result
        except IndexError:
            return self.NA
    ...
    def get_plot_summary(self):
        ...
        movies = db["mini_movies_db"]
        movies_list = [mov for mov in movies.find()]
        for index, i in enumerate(movies_list):
            try:
                if index < 20000:
                    continue
                x = self.get_full_description(i["url"] + "plotsummary/?ref_=tt_ov_pl")
                print(i['name'], len(x))
                print(x)
                print(movies.update_one({"_id": i["_id"]}, {"$set": {'description': x}}))
                print(f'&&&&&&&({index})&&&&&&&')
            except:
                continue
    ...
    def get(self, url):
        ...
        try:
            ...
            result2 = response.html.xpath("//script[@type='application/json']")[0].text

            result = ''.join(result.splitlines())  # removing newlines
            result = f"""{result}"""
            result2 = ''.join(result2.splitlines())  # removing newlines
            result2 = f"""{result2}"""
        ...
        result2 = result2["props"]["pageProps"]["mainColumnData"]["moreLikeThisTitles"]["edges"]
        result2 = [item["node"]["titleText"]["text"] for item in result2]
